...
public class FaceRec{
	...
	public void createEigenFaceImages(Matrix2D eigenfaces,int imgwidth) throws IOException{
		...
		for(int i = 0; i < eigenfacesArray.length; i++){
			...
			String filename = fldrname + "\\" + prefix + i + ext;
			createImageFromArray(filename,egface,imgwidth);
		}
        logger.info("created eigen faces.");
	}
	...
	private void writeCache(String dir,FaceBundle cachedata) throws IOException {		
			...
			ObjectOutputStream fos = null;
				fout = new FileOutputStream(dir +"\\"+"mycache.cache");
	...
	private FaceBundle getOldFacesBundle(String dir) throws IOException, ClassNotFoundException {
		FileInputStream fin = new FileInputStream(dir+"\\"+"mycache.cache");
	...
	private void validateSelectedEigenFacesNumber(int selectedNumOfEigenFaces, List<String> newFileNames) throws FaceRecError {
		int numImgs = newFileNames.size();
		if(selectedNumOfEigenFaces <= 0 || selectedNumOfEigenFaces >= numImgs){
			logger.log(Level.SEVERE,"incorrect number of selected eigen faces " + selectedNumOfEigenFaces+" used "+" allowed btw 0-" + numImgs);
			throw new FaceRecError("incorrect number of selected eigen faces used..\n use a number between 0 and up to " + (numImgs-1));
		}
	}
	...
	private void reconstructOriginalImages(double[][] xnew,String reconFolderName, String ext) throws IOException {
		...
		for(int i = 0; i < xnew.length; i++){
			...
			String filename = reconFolderName + "\\" + prefix + i + ext;
	...
	private int getNumofFacesVal(String numofFaces)throws NumberFormatException{
		return Integer.parseInt(numofFaces);
	}
	...
	private void validateSelectedImageFileName(String faceimagename) throws FaceRecError{
		if ( (faceimagename.length() == 0) || (!isImage(faceimagename)) ){
			throw new FaceRecError("select an image file");
		}
	}
	...
	public MatchResult processSelections(String faceImageName,String directory,int numFaces,double thresholdVal) {
